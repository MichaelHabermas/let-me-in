...
export function mountAuthenticatedAdminEnrollmentCoordinator(
  ...
  const { signal } = ac;
  const adminUi = rt.runtimeSlices.admin.ui;
  ...
  const refreshThresholdStatus = async (): Promise<void> => {
    const t = await readAccessThresholds(persistence, rt.databaseSeedSettings);
    dom.thresholdStatusEl.textContent = formatThresholdStatus(t, adminUi.adminAccessThresholdsStatus);
  };
  ...
  dom.thresholdApplySpec075Btn.addEventListener(
    ...
    () => {
      void (async () => {
        ...
        await refreshThresholdStatus();
      })();
    },
    { signal },
  );

  void refreshThresholdStatus();
