/**
 * Pure embedding similarity and 1:N ranking (no I/O, no infra).
 */

function at32(a: Float32Array, i: number): number {
  const v = a[i];
  if (v === undefined) {
    throw new Error(`Float32Array index ${i} out of range (len ${a.length})`);
  }
  return v;
}
