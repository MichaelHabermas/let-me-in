...
import { l2normalize } from '../src/domain/embedding-match';
...
describe('mountGateIntoHost', () => {
  ...
  it('updates #decision from IndexedDB-backed live access when embedding matches enrollment', async () => {
    ...
    try {
      await vi.waitFor(() => {
        expect(host.querySelector<HTMLButtonElement>('#gate-camera-toggle')!.disabled).toBe(
          false,
        );
      });
      host.querySelector<HTMLButtonElement>('#gate-camera-toggle')!.click();
