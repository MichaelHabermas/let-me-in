...
| | | File size (MB); INT8/FP |
| | | EP used (webgl / wasm) |
| | | ms/frame median (if measured) |
| | | Pass/fail gate zero; fallback triggered? Y/N |
...
| Date | Result | Notes |
| --- | --- | --- |
| | | ms permission → first frame |
| | | Resolution (actual vs ideal 1280×720) |
| | | FPS (canvas loop only) |
