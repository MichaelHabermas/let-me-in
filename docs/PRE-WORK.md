...
| Pure threshold → decision helper                                                | `src/domain/access-policy.ts` (`decideFromMatch`) composed by `src/domain/gate-decision.ts` (`evaluateGateAccessMatch`).                                                                                                                                                                                                                                                                                |
...
| Admin roster + bulk import UI                                                   | `src/app/mount-admin-enrollment.ts` + `src/app/admin-enrollment-mount-coordinator.ts` + `src/app/admin-enrollment-dom.ts` + `src/app/admin-user-roster.ts` + `src/app/bulk-import.ts` (parser in `bulk-import-parse.ts`); E14 SPECS 0.75 `strong` preset: `src/app/admin-threshold-preset.ts` (IndexedDB `settings.thresholds`).                                                                                                                                                                                                                                  |
